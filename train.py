...
def receive_arg():
    ...
    parser.add_argument(
        '--nproc_per_node', type=int, default=0,
        help='Distributed launcher requires.'
    )

    parser.add_argument(
        '--master_port', type=int, default=13254,
        help='Distributed launcher requires.'
    )
...
def main():
    ...
    if opts_dict['train']['phase']=='resume':
        ckp_path = opts_dict['test']['checkpoint_save_path']
    ...
    assert opts_dict['train']['loss'].pop('type') == 'CharbonnierLoss', \
        "Not implemented."
    assert opts_dict['train']['texloss'].pop('type') == 'LogcoshLoss', \
        "Not implemented."
    loss_func = utils.CharbonnierLoss(**opts_dict['train']['loss'])
    loss_tex = utils.LogcoshLoss(**opts_dict['train']['texloss'])
    ...
    #
    if opts_dict['train']['phase'] == 'resume':
        start_iter = opts_dict['test']['restore_iter']
    else:
    ...
    for current_epoch in range(start_epoch, num_epoch + 1):
        ...
        while train_data is not None:
            ...
            ref_data = train_data['ref'].to(rank)
            ...
            loss = torch.mean(torch.stack(
                [(loss_func(enhanced_data[i], gt_data[i])+0.01*loss_tex(enhanced_data[i], gt_data[i]))for i in range(b)]
            ))  # cal loss

...
def paired_random_crop(img_gts, img_lqs, img_ress,gt_patch_size, gt_path, scale=2):
    """Paired random crop.
    ...
    if not isinstance(img_lqs, list):
        img_lqs = [img_lqs]
    if not isinstance(img_ress, list):
        img_ress = [img_ress]
    ...
    if len(img_lqs) == 1:
        img_lqs = img_lqs[0]
    return img_gts, img_lqs,img_ress

def FlipRotate(img_hr,img_lr,img_res):
        hflip = random.random() < 0.5
        vflip = random.random() < 0.5
        rot90 = random.random() < 0.5

        if hflip:
            img_lr = [np.fliplr(img) for img in img_lr]
            img_hr = np.fliplr(img_hr)
            img_res = [np.fliplr(img) for img in img_res]
        if vflip:
            img_lr= [np.flipud(img) for img in img_lr]
            img_hr = np.flipud(img_hr)
            img_res = [np.flipud(img) for img in img_res]
        if rot90:
            img_lr = [img.transpose(1, 0, 2) for img in img_lr]
            img_hr = img_hr.transpose(1, 0, 2)
            img_res =[img.transpose(1, 0, 2) for img in img_res]

        img_lr = [img.clip(0,255) for img in img_lr]
        img_hr = img_hr.clip(0,255)
        img_res =  [img.clip(0,255) for img in img_res]
        return img_hr,img_lr,img_res

...
class SRDataLoader:
    ...
    def get_files(self, phase, samples):
        ...
        for sample_idx, sample_name in enumerate(samples):
            ...
            img_lr_path = self.img_lr_path_template.format(sample_name)
            dims_list = sample_name.split("_")[1]
            width = int(dims_list.split("x")[0])
            height = int(dims_list.split("x")[1])
            frame_num = sample_name.split("_")[2].zfill(3)

            img_hr_name = "%s_%dx%d_%s" % (sample_name.split("_")[0], width * 2, height * 2, frame_num)
            img_hr_path = self.img_hr_path_template.format(img_hr_name)
...
DATASET_LOADER_MAPPING = {
    ...
    'train': SRDataLoader,
    'test': SRDataLoader,
    'avs_train': SRDataLoader,
    'avs_test': SRDataLoader,
    'enhance': SRDataLoader
